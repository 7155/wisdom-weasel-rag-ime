...
final class NativeRoutePolicy {
    ...
    private static let routes: [String: NativeRouteDefinition] = {
        ...
        return [
            "control.events": route("GET", "/api/events/stream", "/control/v1/events", remoteSafe: true, subscription: true),
            ...
            "agent.runtime.ensure": route("POST", "/api/agent/runtime/ensure", "/control/v1/agent/runtime/ensure"),
            "agent.configuration.get": route("GET", "/api/agent/configuration", "/control/v1/agent/configuration", remoteSafe: true),
            "agent.sessions.list": route("GET", "/api/agent/sessions", "/control/v1/agent/sessions", query: ["includeArchived"], remoteSafe: true),
            "agent.sessions.create": route("POST", "/api/agent/sessions", "/control/v1/agent/sessions", remoteSafe: true),
            ...
            "agent.session.rename": route("PATCH", "/api/agent/sessions/{sessionId}", "/control/v1/agent/sessions/{sessionId}", remoteSafe: true, bodyKeys: ["title"]),
            "agent.session.archive": route("PATCH", "/api/agent/sessions/{sessionId}", "/control/v1/agent/sessions/{sessionId}", remoteSafe: true, bodyKeys: ["archived"]),
            "agent.session.mode.update": route("PATCH", "/api/agent/sessions/{sessionId}", "/control/v1/agent/sessions/{sessionId}", bodyKeys: ["mode", "workspaceRoots"]),
            "agent.session.delete": route("DELETE", "/api/agent/sessions/{sessionId}", "/control/v1/agent/sessions/{sessionId}"),
            "agent.session.prompt": route("POST", "/api/agent/sessions/{sessionId}/prompt", "/control/v1/agent/sessions/{sessionId}/prompt", remoteSafe: true),
            ...
            "agent.session.compact": route("POST", "/api/agent/sessions/{sessionId}/compact", "/control/v1/agent/sessions/{sessionId}/compact"),
            ...
            "agent.session.model.select": route("POST", "/api/agent/sessions/{sessionId}/model", "/control/v1/agent/sessions/{sessionId}/model", remoteSafe: true),
            "agent.session.thinking.select": route("POST", "/api/agent/sessions/{sessionId}/thinking", "/control/v1/agent/sessions/{sessionId}/thinking", remoteSafe: true),
            ...
            "agent.rooms.list": route("GET", "/api/agent/rooms", "/control/v1/agent/rooms", query: ["includeArchived"], remoteSafe: true),
            "agent.rooms.create": route("POST", "/api/agent/rooms", "/control/v1/agent/rooms", remoteSafe: true),
            "agent.room.get": route("GET", "/api/agent/rooms/{roomId}", "/control/v1/agent/rooms/{roomId}", remoteSafe: true),
            "agent.room.archive": route("PATCH", "/api/agent/rooms/{roomId}", "/control/v1/agent/rooms/{roomId}", remoteSafe: true, bodyKeys: ["archived"]),
            "agent.room.message": route("POST", "/api/agent/rooms/{roomId}/messages", "/control/v1/agent/rooms/{roomId}/messages", remoteSafe: true),
            ...
            "agent.approvals.list": route("GET", "/api/agent/approvals", "/control/v1/agent/approvals", query: ["sessionId"], remoteSafe: true),
            "agent.approval.get": route("GET", "/api/agent/approvals/{approvalId}", "/control/v1/agent/approvals/{approvalId}", remoteSafe: true),
            "agent.approvals.decide": route("POST", "/api/agent/approvals/{approvalId}/decision", "/control/v1/agent/approvals/{approvalId}/decision", remoteSafe: true),
            "agent.subagents.templates": route("GET", "/api/agent/subagents/templates", "/control/v1/agent/subagents/templates", remoteSafe: true),
            "agent.subagents.list": route("GET", "/api/agent/subagents/runs", "/control/v1/agent/subagents/runs", query: ["sessionId"], remoteSafe: true),
            "agent.subagents.create": route("POST", "/api/agent/subagents/runs", "/control/v1/agent/subagents/runs", remoteSafe: true),
            "agent.subagent.get": route("GET", "/api/agent/subagents/runs/{runId}", "/control/v1/agent/subagents/runs/{runId}", remoteSafe: true),
            "agent.subagent.abort": route("POST", "/api/agent/subagents/runs/{runId}/abort", "/control/v1/agent/subagents/runs/{runId}/abort", remoteSafe: true),
            "agent.memorySources.list": route("GET", "/api/agent/memory-sources", "/control/v1/agent/memory-sources", query: ["sessionId"], remoteSafe: true),
            "planning.dashboard": route("GET", "/api/planning/dashboard", "/control/v1/planning/dashboard", query: ["date"], remoteSafe: true),
            "memory.summary": route("GET", "/api/memory/summary", "/control/v1/memory/summary", remoteSafe: true),
            "memory.pages": route("GET", "/api/memory/{kind}", "/control/v1/memory/{kind}", query: ["limit", "cursor"], remoteSafe: true),
            "history.page": route("GET", "/api/history/page", "/control/v1/history/page", query: ["limit", "cursor"], remoteSafe: true),
            "knowledge.status": route("GET", "/api/knowledge/status", "/control/v1/knowledge/status", query: ["sessionId"], remoteSafe: true),
    ...
    private func resolve(
        ...
        for (name, value) in query where name.utf8.count > 80 || value.utf8.count > 4_096 {
        ...
        if let body {
            ...
            if let allowedBodyKeys = definition.allowedBodyKeys {
                guard let dictionary = body as? [String: Any],
                      !dictionary.isEmpty,
                      dictionary.keys.allSatisfy(allowedBodyKeys.contains) else {
                    throw NativeRoutePolicyError.invalidBody
                }
            }
