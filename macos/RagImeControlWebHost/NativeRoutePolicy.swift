...
final class NativeRoutePolicy {
    ...
    private static let routes: [String: NativeRouteDefinition] = {
        ...
        return [
            ...
            "agent.session.mode.update": route("PATCH", "/api/agent/sessions/{sessionId}", "/control/v1/agent/sessions/{sessionId}", bodyKeys: ["mode", "workspaceRoots", "toolProfileVersion", "toolAllowlistMode", "allowedTools", "dangerousModeConfirmation"], requiredBodyKeys: ["mode"]),
            ...
            "agent.room.message": route("POST", "/api/agent/rooms/{roomId}/messages", "/control/v1/agent/rooms/{roomId}/messages", remoteSafe: true, bodyKeys: ["message", "clientMessageId", "participantIds"], requiredBodyKeys: ["message"]),
