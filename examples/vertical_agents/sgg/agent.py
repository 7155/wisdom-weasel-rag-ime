"""Minimal executable SGG vertical-Agent example.
...
def build_sgg_trace(*, run_id: str = "demo") -> dict[str, object]:
    example_root = Path(__file__).resolve().parent
    manifest = json.loads((example_root.parent / "sgg.json").read_text(encoding="utf-8"))
    document = (example_root / "sales-ledger.md").read_text(encoding="utf-8")
    match = re.search(r"销售额为\s*([0-9.]+)\s*万元", document)
    if match is None:
        raise RuntimeError("SGG ledger contains no sales amount")
    ...
    return build_trace_envelope(
        trace_id=f"trace:vertical:sgg:custom-process:{run_id}",
        source_kind="vertical_agent",
        input_text=str(self_test["query"]),
        binding={
            "sourceLoopId": f"vertical-custom-process:sgg:{run_id}",
        },
        spans=(
            make_span(
                span_id="span:sgg:input",
                name="agent.input",
                started_at_ms=start_ms,
                ended_at_ms=start_ms + 1,
            ),
            make_span(
                span_id="span:sgg:retrieve",
                name="rag.retrieve",
                parent_span_id="span:sgg:input",
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            build_sgg_trace(run_id=args.run_id),
            ensure_ascii=False,
            separators=(",", ":"),
        )
    )
