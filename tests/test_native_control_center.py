...
class NativeControlCenterTests(unittest.TestCase):
    ...
    def test_web_control_center_replaces_the_visible_surface_with_route_and_security_gates(self) -> None:
        ...
        self.assertIn('CONTROL_UI="${RAG_IME_CONTROL_UI:-web}"', build)
        self.assertIn("build-release", build)
        self.assertIn("rollback only", build)
        self.assertIn('CONTROL_UI="${RAG_IME_CONTROL_UI:-web}"', footprint)
        self.assertIn('if [[ "$CONTROL_UI" == "web" ]]', footprint)
        self.assertIn("check_control_center_web_dist.sh", footprint)
        self.assertIn('"$WEB_RESOURCES" native production', footprint)
    ...
    def test_bundle_and_build_script_contract(self) -> None:
        ...
        self.assertIn('RAG_IME_CONTROL_UI:-web', script)
        ...
        self.assertIn("corepack prepare pnpm@11.9.0 --activate", workflow)
