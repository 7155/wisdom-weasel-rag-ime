...
class BuildPatchedSquirrelScriptTests(unittest.TestCase):
    ...
    def test_squirrel_patch_contains_side_first_mixed_layout_hooks(self) -> None:
        ...
        self.assertIn('appendSection("上下文预算"', controller_text)
        self.assertIn("func ragImeActiveRagContextView", patch_text)
        self.assertIn('"value": .string(node.secure ? "" : node.value)', patch_text)
        self.assertIn('response.diagnostics?["contextView"]', patch_text)
        self.assertIn('"contextView": .object(contextView)', patch_text)
        ...
        self.assertIn("let segment = max(0, floor((frame.width - dividerWidth) / 2))", card_text)
        ...
        self.assertIn("actionDivider.layer?.backgroundColor = dividerColor", card_text)
        self.assertIn("let dividerWidth: CGFloat", card_text)
        self.assertIn("x: segment + dividerWidth", card_text)
        ...
        self.assertIn('"com.microsoft.edgemac"', patch_text)
        self.assertIn('"com.google.chrome"', patch_text)
        self.assertIn('"Electron Framework.framework"', patch_text)
        self.assertIn('"Chromium Embedded Framework.framework"', patch_text)
        self.assertIn("func prepareChromiumWebContentAccessibility", patch_text)
        self.assertIn('"AXEnhancedUserInterface" as CFString', patch_text)
        self.assertIn("kAXRoleAttribute as CFString", patch_text)
        self.assertIn('"chromium_accessibility_warmup"', patch_text)
        self.assertIn('privacyStatus.reason == "privacy_unknown_focused_element_missing"', patch_text)
        self.assertIn('"chromium_web_accessibility_warmup"', patch_text)
        self.assertIn('"retryAfterMs": max(80, remainingMs)', patch_text)
        self.assertIn("captureAttempt: 1", patch_text)
        ...
        self.assertNotIn("start_visual_rag_from_context", patch_text)
    ...
    def test_squirrel_patch_declares_active_rag_shortcut_and_endpoint(self) -> None:
        ...
        self.assertNotIn("RagImeVisualContext", patch_text)
        self.assertNotIn("start_visual_rag_from_context", patch_text)
        ...
        quick_end = patch_text.index("func startRagImeAgentDeepSearchFromContext(", quick_start)
        ...
        self.assertIn('"usesScreenCapture": false', quick_path)
