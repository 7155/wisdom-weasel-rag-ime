...
class RoomLearningGovernanceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.store = RoomLearningGovernanceStore(self.db, authority_secrets={"admin:1": b"approval-secret", "user:1": b"user-secret"}, config_secret=b"config-secret", evidence_ttl_ms=1_000); self.assertEqual(self.store.initialize(), 87)
        self._seed_runtime()
