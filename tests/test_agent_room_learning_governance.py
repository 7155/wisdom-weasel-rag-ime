...
class RoomLearningGovernanceTests(unittest.TestCase):
    ...
    def _seed_runtime(self) -> None:
        ...
        with sqlite3.connect(self.db) as conn:
            conn.execute("INSERT INTO room_kernel_roots(root_id,room_id,generation,state,facilitator_participant_id,requirement_anchor_ref,budget_remaining,max_hops,max_depth,payload_json,created_at_ms,updated_at_ms) VALUES ('root:1','room:1',0,'running','owner','anchor:1',10,4,4,'{}',1,1)")
    ...
    def _seed_new_root(self) -> None:
        with sqlite3.connect(self.db) as conn:
            conn.execute("INSERT INTO room_kernel_roots(root_id,room_id,generation,state,facilitator_participant_id,requirement_anchor_ref,budget_remaining,max_hops,max_depth,payload_json,created_at_ms,updated_at_ms) VALUES ('root:new','room:1',0,'running','owner','anchor:1',10,4,4,'{}',101,101)")
