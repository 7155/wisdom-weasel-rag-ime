...
if added_output="$("$CHECK_INPUT_SOURCE_SCRIPT" --require-hitoolbox-enabled "$INPUT_SOURCE_ID" 2>&1)"; then
  echo "Squirrel is already fully added to the current user's macOS input-source lists."
  echo "$added_output"
  echo "Next: switch the macOS input menu to Squirrel - Simplified, then run scripts/wait_squirrel_typing_ready.sh"
  exit 0
fi

echo "Squirrel is not fully added to the current user's macOS input-source lists: $INPUT_SOURCE_ID"
echo "$added_output"
echo
echo "Manual Add flow:"
echo "  System Settings -> Keyboard -> Input Sources -> Add -> Chinese, Simplified -> Squirrel"
echo
...
echo
echo "After adding Squirrel, verify with:"
