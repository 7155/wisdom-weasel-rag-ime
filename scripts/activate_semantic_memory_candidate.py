...
def activate_candidate(
    *,
    ...
    raw_report = (
        ...
        else Path(candidate).expanduser().with_suffix(
            Path(candidate).expanduser().suffix + ".semantic-v2-report.json"
        )
    )
    ...
    if Path(str(report.get("sourcePath") or "")).expanduser().resolve() != target_path:
    ...
    runtime_stop_required = bool(reported_runtime_stop.get("required")) or (
        target_path == default_database
    )
    if runtime_stop_required and not bool(reported_runtime_stop.get("ok")):
        raise ValueError(
    ...
    if expected_source_state != current_source_state:
        raise RuntimeError(
            "target database changed after candidate creation; rebuild from the stopped target"
        )
    ...
    try:
        ...
        payload = {
            ...
            "migrationReportSha256": _sha256_file(resolved_report),
