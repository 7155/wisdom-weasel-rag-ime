...
def main(argv: list[str] | None = None) -> int:
    ...
    organizer = ManagedPiMemoryOrganizer(
        ...
        require_independent_verification=not bool(
            args.defer_batch_verifier
        ),
    )
    try:
...
def _completed_historical_luna_receipts(
    ...
    *,
    ...
    The request ledger is authoritative across process restarts. A projection
    retry may legitimately perform no new model calls, so evaluating only the
    current executor process would erase the earlier Luna/max evidence.
    """
