...
| O4 | PAW can develop itself through a bounded context and Skill harness | active | Core Skills and `pawos-app-builder` exist; 掌柜问数 binds its Pi Package/SGG suite and retains a binding-fault rejection plus linked Sandbox/Trace/Eval verification outside the shell | Install, apply the Package, then verify a foreground 掌柜问数 conversation and uninstall/restore |
