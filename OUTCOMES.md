...
| O4 | PAW can develop itself through a bounded context and Skill harness | active | Core Skills and the source-isolated `pawos-app-builder` exist; 掌柜问数 binds a co-versioned Pi Package and SGG suite, and its source candidate now retains a binding-fault rejection plus linked Sandbox/Trace/Eval verification without putting business code in the shell | Install, apply the Package, then verify a foreground 掌柜问数 conversation and uninstall/restore |
