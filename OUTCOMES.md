...
| O4 | PAW can develop itself through a bounded context and Skill harness | active | Eight conditional core Skills, `bootstrap-project-context`, and Pi-native `plugin-creator` are installed in Codex with matching project-source hashes | Align and document the first PAW OS slice before implementation; verify Pi `/init` from a new installed Session |
...
| O9 | PAW OS reuses Tutti's frontend while Pi/Gateway retain Runtime ownership | active | Private `7155/paw-os` exists at the audited Tutti baseline; the App trunk, input flywheel, Room main/satellite model, themes, product and snapshot seams are accepted; no PAW surface is migrated | Implement and verify the Stage 1 product shell, then migrate Room as the first business App |
