...
RENAME TO room_kernel_runtime_effects_v104;
...
CREATE TABLE room_kernel_runtime_effects (
    ...
    state TEXT NOT NULL
        CHECK(state IN ('intent','accepted','unknown','cancelled','failed')),
...
SELECT
    dispatch_id, root_id, session_id, dispatch_generation,
    state, runtime_receipt_json, updated_at_ms
FROM room_kernel_runtime_effects_v104;

DROP TABLE room_kernel_runtime_effects_v104;

ALTER TABLE room_kernel_abort_scopes
RENAME TO room_kernel_abort_scopes_v104;
...
CREATE TABLE room_kernel_abort_scopes (
    ...
    state TEXT NOT NULL
        CHECK(state IN (
            'registered','cancelling','cancelled','unknown','failed'
        )),
...
SELECT
    dispatch_id, root_id, session_id, generation,
    state, surfaces_json, cancel_receipt_json, updated_at_ms
FROM room_kernel_abort_scopes_v104;

DROP TABLE room_kernel_abort_scopes_v104;
