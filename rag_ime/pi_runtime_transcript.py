...
Both Runtime protocols use this owner for durable branch selection and public
history. Recent windows, append order and stable message/activity identity share
the same projection rules. Inputs are not mutated; loading and validating files,
Host communication, Session state and settlement remain with the caller.

Media URLs use the caller-supplied resolver, as in pi_runtime_public. This module
does not read transcripts or start a Runtime. Cross-module exports are explicit.
"""
