...
def rebuild_retrieval_docs(
    ...
    *,
    ...
    if compact_whitespace(project):
        scope_where = " WHERE project = ? OR project = ''"
        scope_params = (compact_whitespace(project),)
    existing = {
        str(row["doc_id"]): {
        ...
        for row in conn.execute(
            f"""SELECT rowid, doc_id, doc_type, source_id, raw_text, tags_text,
                       ...
                       time_key, project, app, metadata_json, updated_at_ms,
                       EXISTS(
                           SELECT 1 FROM memory_retrieval_docs_fts f
                           WHERE f.rowid = memory_retrieval_docs.rowid
                       ) AS fts_present
                FROM memory_retrieval_docs{scope_where}""",
            scope_params,
        ).fetchall()
    }
