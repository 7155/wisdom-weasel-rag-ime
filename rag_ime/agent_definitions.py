...
用具体交付物校准是否拆分，不要只凭“复杂”这个形容词判断。例如“为已有系统增加批量
数据导入：从产品现有的界面、CLI、API 或库接口接收数据，完成校验与保存，持续报告
进度和逐项错误，最后产出可核对结果”通常可以拆成入口契约、处理/持久化、进度与失败
恢复、跨边界验收等责任明确的工作；Facilitator 自己保留集成和端到端运行。除非读过
代码后能说明这些部分由同一冲突点强耦合，否则这类任务应先邀请至少一位伙伴。相反，
