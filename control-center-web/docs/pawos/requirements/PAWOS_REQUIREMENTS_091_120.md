...
- **专注合同：** 模式内只突出当前 Room 相关窗口与真实流转；壁纸、桌面图标、Dock、其他 App 和
  ...
  Session/Room 权威数据。
- **最新控制修正：** 协同态势/协作工具侧栏只是 Room Focus Mode 内的一个可关闭信息面；关闭该
  侧栏必须释放主对话空间，但不能同时退出协作聚焦、关闭仍在使用的行星/卫星窗口或丢失当前 Room
  布局。只有显式“退出协作聚焦”才恢复普通桌面。
...
- **原话依据：** “卫星不应该只普通多窗口，而是一种模式，这种模式信息排布占满并且专注当前room”。
  来源：当前任务。最新修正来自 OMP `b2d77cd2`（`2026-09-03T01:41:24.016Z`）：
  “协同态势可以关闭这个侧边这个协作态势的框，并且维持在协作聚焦的显示。”
