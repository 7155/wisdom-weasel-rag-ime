...
- **边界：** Trace 诊断阶段保持只读，不能静默修改产品或数据；用户在候选修复
  交接中确认一次后，系统创建独立的普通可写 Agent Session，并默认使用真实
  `full_trust` + `ENABLE_FULL_TRUST` 合同。该 Session 不再逐 Tool 等待用户，
  待审批操作由独立 Luna Max 判定；唯一修复 owner 的工作目录、停止/取消、
  哈希复验、Host 沙箱与硬安全边界始终有效。失败报告不得把明确的 Runtime 原因
  降级成“读取或保存失败，请稍后重试”。
...
- **原话依据：** “利用trace解决读取或保存失败，请稍后重试。任务
  ...
  默认全自动把”。来源：当前任务，2026-08-28、2026-08-30。
