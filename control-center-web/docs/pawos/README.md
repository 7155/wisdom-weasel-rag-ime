...
| Document | Role |
| --- | --- |
| [PAWOS_REQUIREMENTS.md](PAWOS_REQUIREMENTS.md) | Canonical index for the thirteen append-only requirement volumes `UR-001`–`UR-195`, exact-source evidence, product contract, and continuation links. `current` means controlling semantics, not implementation completion. |
| [requirements/](requirements/) | Thirteen stable-ID ledger volumes plus separate verbatim evidence and cross-requirement product contract. Every file links to previous, index, and next. |
