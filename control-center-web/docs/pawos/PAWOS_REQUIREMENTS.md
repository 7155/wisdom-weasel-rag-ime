...
- **状态 / 优先级：** `current / P0`
- **模型菜单：** Composer 底栏显示当前模型与推理强度；点击后使用自绘弹出菜单，
  第一层显示当前模型、推理强度，进入子菜单选择真实可用值，不使用浏览器原生
  `<select>` 下拉。
...
- **状态 / 优先级：** `current / P0`
- **回归基线：** 旧 Agent UI 中模型与推理强度通过锚定浮层分层浏览，模型按
  Provider 分组、选中项清楚、列表独立滚动，Composer 尺寸不受浮层影响；这些
  已正常的交互必须保留并迁入新的 Tutti 工作台骨架。
...
- **身份图标合同：** Project、Agent、Memory、Knowledge、Input、App Center、Monitor、Settings、
  Files、Browser 与 Terminal 不再直接使用通用 Lucide 图标塞入无差别白块。每个 App 使用原创、可辨识
  的 PAWOS 身份剪影，并放入完整、不透明、身份色轻染的圆角方框底座；桌面、Dock、Launchpad、
  窗口标题栏和 App 内身份位复用同一完整资产与名称。
- **视觉合同：** 图标继承当前暖纸/石墨/信号青体系和 Composition 8 的圆、切线、轨道、节点、网格、
  切面等几何语言，但保持 16–48px 下清楚、简洁、对比充分；各 App 有不同轮廓，笔画、基底、信号点、
  选中态和主题适配一致。不得复制游戏、Chrome、Ghostty 或其他产品商标。
...
- **原话依据：** “图标也得全部重新制作，因为和当前风格完全不符合”。来源：当前任务及附件截图。
...
### UR-103 — 保留独立 App 剪影，并补齐完整 OS 图标底座

- **状态 / 优先级：** `current / P0 correction`
- **废弃合同：** 当前把 11 个 App 细线符号塞入同一折角工程牌、底轨与信号条的方案被明确拒绝，
  不再通过改色、加粗描边或微调比例抢救；折角、底轨或同一抽象符号不能继续成为身份图标主体。
- **独立剪影合同：** 每个 App 用 2–4 个清楚的实心几何形建立独立外轮廓，最多保留一个光学细节；
  统一只来自石墨/暖纸/单一 App 强调色、光学边界、对比度和有限的笔画体系。Agent 必须是系统连接
  关系而非脸或头像。16/24/32/48px 下都要直接可辨识，动作图标仍与身份图标分层。
- **最新底座修正：** App 启动身份不能继续是透明地面上的裸剪影。所有 App 使用同一完整圆角方框几何，
  以各自 APP_COLORS 身份色轻染且保持不透明，并由原有独立剪影负责语义区分。Dock、Launchpad、
  桌面快捷方式必须显示完整底座；菜单栏和标题栏可缩小但不能退回裸图形。Hover、选中和运行态只提升/
  提亮底座，不在外面再套第二张白卡。
...
- **原话依据：** “我的天呐，图标还能丑到这种地步”“太丑了，我都想不到怎么能画出这么丑的图标”。
  最新修正：“icon得优化，不符合os。得有完整的方框，现在就透明的”。来源：当前任务。
...
- **图标修正合同：** UR-103 的独立轮廓继续有效，但“来自 Composition 8”不再是设计理由。接触表
  ...
  与光学深度。各 App 共享完整、不透明的 OS 底座几何，但底座身份色、内部剪影和语义轮廓必须保持
  区分，小尺寸仍清晰且不复制其他品牌。
...
中本文件所摘录的相关用户消息，均已映射到 `UR-001`–`UR-131`；重复强调没有
