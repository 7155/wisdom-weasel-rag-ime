...
const WORD_CHAR = /[\w-]/;
function avoidBrokenSurrogate(text, offset) {
...
export function findSafeInlineBoundary(text, limit = text.length) {
    ...
    if (openBacktickAt >= 0)
        boundary = Math.min(boundary, openBacktickAt);
    if (unresolvedBracketAt >= 0) {
 ...
 */
export function computeReleaseCeiling(text, maxHoldBackChars = 600) {
    const safe = findSafeInlineBoundary(text);
    return avoidBrokenSurrogate(text, Math.max(safe, text.length - maxHoldBackChars));
}
export function advanceToSafeBoundary(text, current, ceiling, stepChars = 40) {
    if (current >= ceiling)
        return ceiling;
...
//# sourceMappingURL=safeInlineBoundary.js.map