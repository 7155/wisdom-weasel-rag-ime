...
export const INITIAL_SCAN_CURSOR = Object.freeze({
    codeFence: null,
    inMathBlock: false,
    inList: false,
...
export const INITIAL_SCAN_STATE = Object.freeze({
...
function insideAnyBlock(cursor) {
    return (cursor.codeFence !== null ||
        cursor.inMathBlock ||
        cursor.inList ||
        cursor.inTable ||
        cursor.inBlockquote ||
 ...
 */
function scanSuffix(text, cursor) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        const fenceMatch = isIndented || inMathBlock || inIndentedCode
            ? null
        ...
        if (isBlank) {
            ...
            if ((!wasInsideAnyBlock && hadContent) ||
 ...
 */
export function scanIncrementalMarkdown(previous, text, enabled = true) {
    if (!enabled || text.length === 0)
        return INITIAL_SCAN_STATE;
    if (text === previous.previousText)
        return previous;
    const base = text.startsWith(previous.previousText)
        ? previous
        : INITIAL_SCAN_STATE;
    const { boundaries, nextCursor } = scanSuffix(text, base.cursor);
    const newBoundaries = boundaries.filter((boundary) => boundary > base.committedEnd);
...
export function splitSettledMarkdown(text) {
    if (text.length === 0)
        return [];
...
export function resolveProgressiveChunks(text, isStreaming, hasEverStreamed, scanState) {
    if (isStreaming) {
        return {
...
//# sourceMappingURL=blockScanner.js.map