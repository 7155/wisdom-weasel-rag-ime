...
function collectBacktickFences(text) {
    const tokens = [];
    const expression = /^([^\S\n\r\u2028\u2029]*)(`{3,})(.*)$/gm;
 ...
 */
function normalizeNestedBacktickFences(text, mode) {
    const first = text.indexOf("```");
    if (first < 0 || text.indexOf("```", first + 3) < 0)
        return text;
    ...
    for (const token of tokens) {
        ...
        maxWidthByDepth.set(token.depth, Math.max(maxWidthByDepth.get(token.depth) ?? 3, token.width));
    }
    for (let depth = deepest - 1; depth >= 0; depth -= 1) {
        const own = maxWidthByDepth.get(depth) ?? 3;
        const child = maxWidthByDepth.get(depth + 1) ?? 0;
        if (child >= own)
            maxWidthByDepth.set(depth, child + 1);
    }
    const replacements = [];
    for (const token of tokens) {
    ...
    for (const replacement of replacements)
        unique.set(replacement.start, replacement);
...
function removeFenceIndentation(text) {
    return text.replace(/^([^\S\n\r\u2028\u2029]*)(`{3,})/gm, "$2");
}
function hideIncompleteSetextUnderline(text) {
 ...
 */
export function normalizeStreamingMarkdown(input, options = {}) {
    const { isStreaming = false, nestedCodeBlockMode = "code-in-markdown", } = options;
    ...
    if (isStreaming)
        text = hideIncompleteSetextUnderline(text);
    return text;
}
//# sourceMappingURL=normalizeStreamingMarkdown.js.map