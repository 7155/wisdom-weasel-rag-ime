...
export function rememberRootChildren(root, file) {
 ...
 */
export function restoreInsertedRootPositions(root, file) {
 ...
 */
export function splitPositionedChildrenByOffsets(children, chunkOffsets) {
    const buckets = chunkOffsets.map(() => []);
    if (buckets.length === 0)
        return buckets;
    const nextPositionedOffset = Array.from({ length: children.length }, () => Number.POSITIVE_INFINITY);
    ...
    children.forEach((node, index) => {
        ...
        if (typeof offset === "number") {
            while (bucket + 1 < chunkOffsets.length &&
...
//# sourceMappingURL=positionedTree.js.map