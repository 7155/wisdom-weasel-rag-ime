...
import { advanceToSafeBoundary, computeReleaseCeiling, } from "../core/safeInlineBoundary.js";
const useBrowserLayoutEffect = typeof window === "undefined" ? useEffect : useLayoutEffect;
function clamp(value, minimum, maximum) {
    return Math.min(maximum, Math.max(minimum, value));
}
function commonPrefixLength(left, right, maximum = Math.min(left.length, right.length)) {
    let index = 0;
 ...
 */
export function useSafeTextRelease(text, options) {
    const { enabled, stepChars = 40, minimumIntervalMs = 25, maximumIntervalMs = 150, backlogBudgetMs = 12_000, maximumHoldBackChars = 600, } = options;
    ...
    const ceilingRef = useRef(enabled ? computeReleaseCeiling(text, maximumHoldBackChars) : text.length);
    useBrowserLayoutEffect(() => {
        ...
        if (!enabled) {
            ...
            nextVisibleEnd = commonPrefixLength(previousText, text, previousVisibleEnd);
        }
        nextVisibleEnd = Math.min(nextVisibleEnd, ceiling, text.length);
        if (enabled && nextVisibleEnd === 0 && ceiling > 0) {
            nextVisibleEnd = advanceToSafeBoundary(text, 0, ceiling, Math.min(120, Math.max(stepChars, ceiling)));
        }
        sourceRef.current = text;
        ceilingRef.current = ceiling;
        visibleEndRef.current = nextVisibleEnd;
        if (nextVisibleEnd !== visibleEnd)
            setVisibleEnd(nextVisibleEnd);
    ...
    useEffect(() => {
        ...
        const tick = (now) => {
            ...
            const interval = clamp(backlogBudgetMs / backlog, minimumIntervalMs, maximumIntervalMs);
            if (now - lastAdvanceAt < interval)
                return;
            lastAdvanceAt = now;
            const next = advanceToSafeBoundary(source, current, ceiling, stepChars);
            if (next > current) {
...
//# sourceMappingURL=useSafeTextRelease.js.map