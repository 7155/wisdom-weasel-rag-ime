...
| Agent Lab：Agent 主导的通用项目与动态成果 | `PawNativeApps.tsx`；`features/eval-lab/index.tsx`；`features/eval-lab/projects/{LabProjectWorkbench,ArtifactSurface,ProjectGuide,LabAppDelivery,LabAppPreview,LabAppHost}.tsx` | `renderApp('eval-lab') -> PawNativeApp -> EvalLabFeature -> LabProjectWorkbench` | `/eval-lab`；项目自定义成果/视图、材料版本、同源 Pi 对话及执行绑定；版本化 App 预览、PAW 启用/回退及双目标包交付；已有 Golden/实验与诊断链接保留。源码选择不证明安装、真实实验或 App 导出通过 |
