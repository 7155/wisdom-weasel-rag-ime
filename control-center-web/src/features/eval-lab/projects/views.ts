export type ProjectPage = 'artifact' | 'materials' | 'runs' | 'brief' | 'apps' | 'knowledge' | 'lifecycle';
...
export function readProjectViews(connection: string): Record<string, ProjectView> {
  try {
    ...
    return Object.fromEntries(Object.entries(raw).filter(([,value]) => value && typeof value === 'object'
      && ['artifact','materials','runs','brief','apps','knowledge','lifecycle'].includes(value.page) && typeof value.guideOpen === 'boolean'
