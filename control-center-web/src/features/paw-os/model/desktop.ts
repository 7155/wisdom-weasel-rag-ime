...
export function createPawOsWindow(input: {
  ...
  const width = compact ? Math.max(320, input.surface.width - 20) : Math.min(1080, input.surface.width - 350);
  const height = compact ? Math.max(300, input.surface.height - 28) : Math.min(720, input.surface.height - 72);
