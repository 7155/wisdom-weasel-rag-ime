  ...
  return <>
    <section aria-label="Trace 诊断网页报告" className="trace-audit" data-status={model.status} data-testid="trace-agent-web-report" data-tone={model.verdict.tone}>
      ...
      <details className="trace-audit__appendix"><summary><span><strong>证据详情与技术附录</strong><small>原记录、诊断归因、时间线、版本和评分</small></span><span className="trace-audit__details-affordance">展开明细</span></summary><div className="trace-audit__appendix-body">
        ...
        <ScanSection className="trace-audit__knowledge" description="把已确认事实和仍缺少的证据放在一起，避免把未知写成结论。" id="knowledge" title="支持这个判断的事实">
          <div className="trace-audit__knowledge-columns"><section aria-labelledby="trace-audit-known-title"><h4 id="trace-audit-known-title">已知事实</h4><ul>{model.knownFacts.map((fact) => <li key={fact}>{fact}</li>)}</ul></section><section aria-labelledby="trace-audit-gaps-title"><h4 id="trace-audit-gaps-title">证据缺口</h4><ul>{model.evidenceGaps.map((gap) => <li key={gap}>{gap}</li>)}</ul></section></div>
        </ScanSection>

        <ScanSection className="trace-audit__actions-section" description="先确认修复授权，再读取修复 Trace 中已记录的修改与测试证据，由 AI Judge 复检；不声称在复检中重跑命令或进行同案 Trace 回放。" id="actions" title="下一步怎么做">
        ...
        {model.gates.length ? <AppendixSection id="gates" title="硬门槛"><div className="trace-audit__gates">{model.gates.map((gate) => <article data-status={gate.status} key={gate.gateId}><strong>{gate.gateId}</strong><span>{gate.statusLabel}</span><p>{gate.reason}</p><EvidenceIds aliases={model.evidenceAliases} full onOpen={openEvidence} values={gate.evidenceIds} /></article>)}</div></AppendixSection> : null}
        <AppendixSection id="metrics" title="扫描指标"><dl className="trace-audit__metrics">{model.metrics.map((metric) => <div data-available={metric.available} key={metric.key}><dt>{metric.label}</dt><dd>{metric.value}</dd><small>{metric.detail}</small></div>)}</dl></AppendixSection>
        <AppendixSection id="timeline" title="跨 Session / Agent 因果时间线 · 查看完整冻结时间线 · 完整时间线">{model.timeline.length ? <TimelineList aliases={model.evidenceAliases} items={model.timeline} onOpenEvidence={openEvidence} /> : <p className="trace-audit__empty">没有公开时间线。</p>}</AppendixSection>
        <AppendixSection id="environment" title="可复现环境快照 · 环境快照"><Environment model={model} /></AppendixSection>
        <AppendixSection id="dimensions" title={`八维诊断评分 · 八维评分明细（${model.dimensionSummary}）`}><DimensionTable aliases={model.evidenceAliases} model={model} onOpenEvidence={openEvidence} /></AppendixSection>
        <AppendixSection id="repair" title="修复授权状态"><RepairState model={model} /></AppendixSection>
        <AppendixSection id="comparison" title="修复前后 Trace / Eval 对照 · 修复对照"><Comparison model={model} onOpenTrace={onOpenTrace} /></AppendixSection>
        <AppendixSection id="evidence" title="Evidence 目录"><div className="trace-audit__evidence-catalog-list">{model.evidence.length ? model.evidence.map((item) => <button key={item.evidenceId} onClick={() => openEvidence(item.evidenceId)} type="button"><code>[{item.alias}] {item.evidenceId}</code><span>{item.summary}</span></button>) : <p className="trace-audit__empty">冻结报告不包含可公开展示的 Evidence。</p>}</div>{model.unresolvedEvidenceIds.length ? <p className="trace-audit__unresolved">{model.unresolvedEvidenceIds.length} 个引用未解析，未尝试从实时系统补齐。</p> : null}</AppendixSection>
      </div></details>

      <div aria-label="报告操作" className="trace-audit__links" role="toolbar">{exportHref ? <a className="trace-audit__download" download="trace-diagnostic-report.html" href={exportHref}><Download aria-hidden="true" size={16} />下载 HTML 报告</a> : <span aria-disabled="true" className="trace-audit__download">HTML 导出不可用</span>}<Button leadingIcon={<ShieldCheck size={16} />} onClick={onOpenDiagnosticSession} size="small">打开诊断 Agent 对话</Button>{report.targets.slice(0, 3).map((target) => <Button key={target.targetKey} leadingIcon={<ArrowUpRight size={16} />} onClick={() => onOpenTarget(target)} size="small" variant="quiet">打开{target.kind === 'room' ? ' Room' : target.kind === 'run' ? '运行记录' : ' Session'}</Button>)}</div>
...
function ScanSection({ children, className, description, id, title }: { children: ReactNode; className: string; description: string; id: string; title: string }) { const headingId = `trace-audit-${id}-title`; return <section aria-labelledby={headingId} className={`trace-audit__section ${className}`}><header className="trace-audit__section-heading"><h3 id={headingId}>{title}</h3><p>{description}</p></header>{children}</section>; }
function AppendixSection({ children, id, title }: { children: ReactNode; id: string; title: string }) { const headingId = `trace-audit-appendix-${id}-title`; return <section aria-labelledby={headingId} className="trace-audit__appendix-section"><h3 id={headingId}>{title}</h3>{children}</section>; }
...
function Environment({ model }: { model: TraceAuditReportModel }) { return <><p className="trace-audit__appendix-status">{model.environment.statusLabel} · 捕获于 {model.environment.capturedAtLabel}</p>{model.environment.targets.length ? <div className="trace-audit__environment-targets">{model.environment.targets.map((target) => <article key={target.targetKey}><h4>{target.targetLabel}</h4><dl><Meta label="Runtime" value={target.runtime} /><Meta label="模型配置" value={target.modelProfile} /><Meta label="Tool 配置" value={target.toolProfileVersion} /><Meta label="执行模式" value={target.executionMode} /><Meta label="策略修订" value={target.policyRevision} /><Meta label="Shell 策略" value={target.shellPolicyVersion} /><Meta label="源快照 SHA-256" value={target.sourceSha256} mono /><Meta label="Workspace scope SHA-256" value={target.workspaceScopeSha256} mono /><Meta label="Trace input fingerprint" value={target.traceInputFingerprints.join(' · ') || '未冻结'} mono /><Meta label="Trace 状态" value={target.traceStatuses.join(' · ') || '未知'} /></dl></article>)}</div> : <p className="trace-audit__empty">没有 Runtime 权威环境数据，不能声称可复现。</p>}{model.environment.limitations.length ? <ul className="trace-audit__limitations">{model.environment.limitations.map((item) => <li key={item}>{item}</li>)}</ul> : null}</>; }
function DimensionTable({ aliases, model, onOpenEvidence }: { aliases: Record<string, string>; model: TraceAuditReportModel; onOpenEvidence: (evidenceId: string) => void }) { return <div className="trace-audit__score-table-wrap" data-testid="trace-diagnostic-scorecard"><table className="trace-audit__score-table"><thead><tr><th>维度与适用性</th><th>系统分数</th><th>AI 估计</th><th>指标与证据边界</th></tr></thead><tbody>{model.dimensions.map((dimension) => <tr data-applicability={dimension.applicability} data-dimension-id={dimension.dimensionId} key={dimension.dimensionId}><th scope="row"><strong>{dimension.title}</strong><span>{dimension.applicabilityLabel}</span></th><td><strong>{dimension.scoreText}</strong><span>{dimension.authorityLabel}</span></td><td><strong>{dimension.applicability === 'not_applicable' ? '不评分' : dimension.judgeScore === null ? '没有估计' : `${dimension.judgeScore}/3`}</strong>{dimension.judgeExplanation ? <span>{dimension.judgeExplanation}</span> : null}</td><td>{dimension.metrics.length ? dimension.metrics.map((metric) => <span key={`${dimension.dimensionId}:${metric.label}`}><strong>{metric.label}</strong> {metric.value}</span>) : <span>{dimension.note}</span>}<EvidenceIds aliases={aliases} full onOpen={onOpenEvidence} values={dimension.evidenceIds} /></td></tr>)}</tbody></table></div>; }
function Comparison({ model, onOpenTrace }: { model: TraceAuditReportModel; onOpenTrace?: (traceId: string) => void }) {
  ...
  return <div className="trace-audit__comparison" data-status={repair.comparisonStatus}><header><div><span>验证状态</span><strong>{repair.verificationStateLabel}</strong></div><div><span>同 Case 决策</span><strong>{repair.decision === 'kept' ? 'Keep' : repair.decision === 'rejected' ? 'Reject' : '尚无回执'}</strong></div><div><span>测试证据</span><strong>{repair.testStatus}</strong></div><div><span>{testAuthorityLabel}</span><strong>{testAuthorityValue}</strong></div></header><dl className="trace-audit__comparison-refs"><MetaAction label="诊断 Trace" onClick={repair.sourceTraceId && onOpenTrace ? () => onOpenTrace(repair.sourceTraceId) : undefined} value={repair.sourceTraceId || '未绑定'} /><MetaAction label="修复 Trace" onClick={repair.repairTraceId && onOpenTrace ? () => onOpenTrace(repair.repairTraceId) : undefined} value={repair.repairTraceId || '等待修复 Trace'} /><Meta label="Replay Case" value={repair.replayCaseId || '尚未冻结'} mono /><Meta label="Verification Receipt" value={repair.verificationReceiptId || '尚未生成'} mono /><Meta label="AI Judge EvalRun" value={repair.evalRunId || '等待 AI Judge Eval'} mono /><Meta label="修复回执" value={repair.repairReceiptId || '尚未生成'} mono /></dl><p>{repair.comparisonReason}</p>{repair.comparisonMetrics.length ? <div className="trace-audit__comparison-table-wrap"><table><thead><tr><th>指标</th><th>Before</th><th>After</th><th>Δ</th></tr></thead><tbody>{repair.comparisonMetrics.map((metric) => <tr key={metric.metricId}><th scope="row">{metric.metricId}</th><td>{metric.before}</td><td>{metric.after}</td><td>{metric.delta}</td></tr>)}</tbody></table></div> : <p className="trace-audit__empty">还没有可展示的修复前后指标。</p>}</div>;
}
