...
function renderComparison(model: TraceAuditReportModel): string {
  ...
  return `<div class="trace-audit__comparison" data-status="${attr(repair.comparisonStatus)}"><header><div><span>验证状态</span><strong>${escapeHtml(repair.verificationStateLabel)}</strong></div><div><span>可比性</span><strong>${escapeHtml(repair.comparisonStatusLabel)}</strong></div><div><span>测试证据</span><strong>${escapeHtml(repair.testStatus)}</strong></div><div><span>${escapeHtml(testAuthorityLabel)}</span><strong>${escapeHtml(testAuthorityValue)}</strong></div></header><dl class="trace-audit__comparison-refs">${meta('诊断 Trace', repair.sourceTraceId || '未绑定', true)}${meta('修复 Trace', repair.repairTraceId || '等待修复 Trace', true)}${meta('AI Judge EvalRun', repair.evalRunId || '等待 AI Judge Eval', true)}${meta('修复回执', repair.repairReceiptId || '尚未生成', true)}</dl><p>${escapeHtml(repair.comparisonReason)}</p>${metrics}</div>`;
}
