...
describe('Knowledge write boundaries', () => {
  ...
  it('binds database apply preview to its receipt and rollback token', async () => {
    ...
    const transport = renderKnowledge();
    await screen.findByRole('heading', { name: '个人知识整理', level: 2 });
    await user.type(await screen.findByPlaceholderText('输入一个明确的知识任务'), '整理现有知识');
    const startWorkflow = screen.getByText('启动知识任务', { selector: 'strong' }).closest('.mgmt-workflow');
    expect(startWorkflow).not.toBeNull();
    await user.click(within(startWorkflow as HTMLElement).getByRole('button', { name: '预览任务' }));
    await user.click(within(startWorkflow as HTMLElement).getByRole('button', { name: '进入确认' }));
    await user.click(within(startWorkflow as HTMLElement).getByRole('checkbox'));
    await user.click(within(startWorkflow as HTMLElement).getByRole('button', { name: '确认启动' }));
