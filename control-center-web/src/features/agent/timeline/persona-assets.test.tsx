...
describe('Persona timeline assets', () => {
  it('maps all four fixed roles onto distinct portraits', () => {
    expect(personaAssetManifest.schemaVersion).toBe('rag-ime.persona-assets.v8');
    ...
    expect(resolvePersonaAsset('rag-ime-timeline-past-v1', 'thinking')).toBe('/companions/personas/companion-firstlight-v4.webp');
    expect(resolvePersonaAsset('rag-ime-timeline-present-v1', 'done')).toBe('/companions/personas/companion-present-v4.webp');
    expect(resolvePersonaAsset('rag-ime-timeline-future-v1', 'idle')).toBe('/companions/personas/companion-future-v4.webp');
    expect(resolvePersonaAsset('rag-ime-timeline-flash-v1', 'listening')).toBe('/companions/personas/companion-flash-v4.webp');
    expect(new Set([
  ...
  it('fails soft to the present timeline and resolves stickers into the current art pack', () => {
    expect(resolvePersonaAsset('future-persona-v9', 'warning')).toBe('/companions/personas/companion-present-v4.webp');
    expect(stickerAsset('rag-ime-presence-warning')).toBe('/companions/personas/companion-present-v4.webp');
  });
