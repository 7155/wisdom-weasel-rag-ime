...
describe('room gravity projection over the minecraft harness', () => {
  ...
  it('renders the collaboration console as one mesh with owners, blockers and verifiers', () => {
    ...
    // The partner-only graph: every real planet is exactly one clickable mesh
    // node; Sol stays chrome and WorkItems stay inspectable data (UR-023).
    ...
    // The real parallel wave keeps its two lanes readable in the console —
    // the flow ledger carries each dispatch with its lane once the recent
    // window is expanded to the full history.
    fireEvent.click(screen.getByRole('button', { name: '显示全部' }));
    expect(screen.getAllByText(/并行轨道 1\/2/).length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText(/并行轨道 2\/2/).length).toBeGreaterThanOrEqual(1);
    ...
    // Edges exist only for recorded partner-to-partner relations: one review
    // edge per distinct recorded verifier→owner pair in this fixture.
    const partnerIds = new Set(harness.focus.partners.map((partner) => partner.participantId));
    const reviewPairs = new Set(harness.focus.workItems.flatMap((item) => {
      const reviewer = item.review?.reviewerParticipantId || item.verifierParticipantId;
      const owner = item.ownerParticipantId || item.offeredToParticipantId;
      return reviewer && owner && reviewer !== owner && partnerIds.has(reviewer) && partnerIds.has(owner)
        ? [`${reviewer}->${owner}`]
        : [];
    }));
    expect(container.querySelectorAll('.paw-room-focus-overview__mesh-edge[data-kind="review"]')).toHaveLength(reviewPairs.size);

    // The blocked task is still the strongest state, so it is the default
    // selection and its real blocker reads in the inspector.
    const inspector = screen.getByRole('region', { name: '焦点详情' });
    expect(inspector).toHaveAttribute('data-state', 'blocked');
    expect(inspector).toHaveTextContent('WorkDocument 尚未完成开工与交付同步');
