 ...
 * instead of a uniform blue haze. Cumulative weight → linear RGB 0..1.
 */
const SPECTRAL_STAR_TYPES: ReadonlyArray<{ cumulative: number; rgb: readonly [number, number, number] }> = [
...
export function spectralStarColor(unit: number): readonly [number, number, number] {
  ...
  for (const type of SPECTRAL_STAR_TYPES) {
    if (pick <= type.cumulative) return type.rgb;
  }
  return SPECTRAL_STAR_TYPES[SPECTRAL_STAR_TYPES.length - 1]!.rgb;
}
