...
describe('PAWOS semantic type roles', () => {
  ...
  it('uses one bounded motion contract without clipping open nested Tool evidence', () => {
    // The shared disclosure voice is the 220ms conversation spring
    // (DISCLOSURE_MOTION in SmoothDisclosureReveal.tsx / --paw-chat-spring).
    ...
    expect(agentFxCss).toMatch(
      /\.agent-turn-work__reveal\s*\{[^}]*height 220ms var\(--paw-chat-spring, cubic-bezier\(\.34, 1\.4, \.64, 1\)\)/s,
    );
    expect(agentFxCss).toMatch(
      /\.paw-activity__detail\s*\{[^}]*height 220ms var\(--paw-chat-spring, cubic-bezier\(\.34, 1\.4, \.64, 1\)\)/s,
    );
    expect(agentFxCss).toMatch(
