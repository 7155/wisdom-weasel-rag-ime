...
function readStoredTheme(): PawOsTheme {
  if (typeof window === 'undefined') return 'glacier';
  const stored = window.localStorage.getItem(PAW_OS_THEME_STORAGE_KEY);
  return isPawOsTheme(stored) ? stored : 'glacier';
}

function isPawOsTheme(value: string | null): value is PawOsTheme {
  return value === 'glacier' || value === 'ink-paper' || value === 'blueprint';
}

function setPawOsTheme(next: PawOsTheme): void {
  window.localStorage.setItem(PAW_OS_THEME_STORAGE_KEY, next);
