...
The sidecar exposes `/rime-suggest` for merged Rime/RAG/model candidates and `/rime-select` for accepted side-candidate feedback. `/rime-select` records the committed text, applies accepted feedback to the newly committed side candidate, records source-memory feedback when a RAG/memory candidate carries memory ids, and can use `shownCandidates` to mark higher-ranked side candidates as skipped.
