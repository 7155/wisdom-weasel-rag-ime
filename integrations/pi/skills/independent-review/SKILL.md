...
- Review is a post-integration delegation to a distinct formal Room partner.
  Give it one fixed scope, acceptance criteria, evidence refs, and the actual
  workspace mode; a filesystem root is not participant identity.
...
- A review before integration is invalid. The Reviewer returns findings through
  its Session result; missing, stale, failed, or ineligible evidence means
  `changes_required`, wait, or blocked, never a prose-based pass.
- Keep private Session reasoning and refs private. Only the Facilitator emits
  the Room's final public summary.
...
These are findings for the Facilitator, not an automatic verdict.
