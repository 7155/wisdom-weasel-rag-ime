...
1. Read the project, outcome, precise requirement, Room brief, current workboard, and Runtime participant projection. Inventory unfinished, failed, orphaned, partial, and unclosed WorkItems before creating more. A natural complex-project request is enough to begin; do not ask the user to restate the workflow. Before any requirements, business-code, configuration, or test write and before any `room_partner delegate` / `delegate_batch`, establish the Root authority and bind its WorkDocument:
   ...
   - Continue only after the write receipt includes a successful `workDocumentRegistration` whose `authorityKey` is `session_goal:<goalId>`; keep its `documentId`, canonical path, and revision for every delegation and closeout. The model-facing registration seam is the write's `workDocument` field; do not omit the binding because a standalone `work_documents register` Tool is absent from the current profile.
...
5. Let each Partner choose private Session subagents within its granted capabilities. Dispatch an artifact-dependent private Reviewer only after the producer's artifact and evidence exist; never place that Reviewer in the producer's wave.
...
7. Integrate Partner results into the authoritative workspace and shared workboard. For each submitted WorkItem, inspect current evidence and revision, then judge separately whether the implementation/real path runs and whether the observed result satisfies the current precise requirement. The Facilitator must explicitly call `accept` or `return` with `expectedRevision`, both verdicts, and `evidenceRefs`. Accept only with `operabilityVerdict=passed` and `requirementVerdict=satisfied`; otherwise return it with a concrete `reason` and responsible next action. To send a returned item back for revision, make a new `delegate` Tool call carrying the original `workItemId` and the unchanged bounded delivery contract; do not create a replacement WorkItem for the same revision chain.
...
9. Reconcile every WorkItem to `complete`, `partial`, `blocked`, `abandoned`, or an explicit unresolved result with owner, evidence, next action, and document receipt. The Root Agent alone merges the shared index and owns the integrated terminal result. Then update the Room final document and emit one integrated final result. Runtime remains responsible for enforcing the unique terminal event.
...
Do not reproduce Room rules inside task Skills, require a Kernel formatting gate, manufacture review, treat Partner prose as automatic acceptance, or leave an unfinished/unowned WorkItem hidden behind the Room final.
