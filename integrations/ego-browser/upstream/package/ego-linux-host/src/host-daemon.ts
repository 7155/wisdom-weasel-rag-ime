...
export async function startDaemon(
  ...
  const pawBridgeRequest = async (pathname: string, payload: object): Promise<any> => {
    const origin = env.EGO_PAW_HOST_ORIGIN;
    const token = env.EGO_PAW_HOST_TOKEN;
    if (!origin || !token) throw makeEgoError("EGO_WEB_CONTENTS_UNAVAILABLE", "PAW Browser host bridge is unavailable");
    const response = await fetch(new URL(pathname, origin), {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Paw-Browser-Token": token },
      body: JSON.stringify(payload),
    });
    const result = await response.json();
    if (!response.ok || !result?.ok) throw makeEgoError("EGO_WEB_CONTENTS_UNAVAILABLE", String(result?.error || "PAW Browser host rejected the request"));
    return result;
  };
  ...
  const ensureSession = async (): Promise<string> => {
    ...
    const active = inSpace.find((page) => page.active) ?? inSpace[inSpace.length - 1];
    if (!active) {
      throw makeEgoError(
  ...
  const runtime = createEgoRuntime({
    ...(env.EGO_PAW_HOST_ORIGIN && env.EGO_PAW_HOST_TOKEN ? {
      activateVisibleTarget: async (targetId: string) => {
        await pawBridgeRequest("/__paw_browser/activate", { targetId });
      },
      createVisibleTab: async (url: string) => {
        const result = await pawBridgeRequest("/__paw_browser/tabs", { url });
    ...
    getCdp,
    ensureSession,
    reuseSelectedTarget: env.EGO_PAW_REUSE_SELECTED_TARGET === "1",
