...
function setup(opts?: { targets?: CdpPageTarget[]; reuseSelectedTarget?: boolean }) {
  ...
  const runtime = createEgoRuntime({
    ...
    getCdp: () => fakeCdp,
    ensureSession,
    reuseSelectedTarget: opts?.reuseSelectedTarget,
  });
...
test("PAW Electron task space adopts and navigates the visible guest target", async () => {
  const { sm, fakeCdp, runtime } = setup({
    reuseSelectedTarget: true,
    targets: [{ targetId: "visible", title: "PAW Browser", url: "about:blank", type: "webview" }],
  });
  const created = await runtime.handle("createTaskSpace", { name: "shared-page" });
  await runtime.handle("useTaskSpace", { id: created.id });

  const opened = await runtime.handle("createTab", { url: "https://example.com" });

  assert.equal(opened.targetId, "visible");
  assert.deepEqual(sm.targetsForSelected(), ["visible"]);
